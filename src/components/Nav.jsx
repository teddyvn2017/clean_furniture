...
const Nav = () => {
    ...
    const toggleMenu = () => {
        ...
        if (!isOpen) {
            ...
            //     timeline.to(menuItemRefs.current[i], { opacity: 1, y: -20, duration:1, ease: 'power2.easeInOut' }, `-=${0.3 - (i * 0.1)}`);
            //     console.log(i);
            // }
            console.log(isOpen);
            // console.log( menuItemRefs.current.length);
        }
        else {
            // gsap.to(humburgerRef.current, { rotation: 0, duration: 0.5, ease: 'power3.in' });
            // menuItemRefs.current.forEach((menuItem) => {
            //     timeline.to(menuItem, { duration: 0.4, opacity: 0, y:20,ease: 'power2.easeInOut'},'-=0.1' );
            // });
            // gsap.to(menuRef.current, { opacity: 0,  duration: 2, ease: 'power2.easeInOut', onComplete: () => setIsOpen(false) });
            ...
            // tạm được di chuyển từ phần tử đầu tiên
            // menuItemRefs.current.forEach((menuItem, index) => {
            //     timeline.to(menuItem, { duration: 0.4, opacity: 0, y: 0, ease: 'power2.easeInOut' },'-=0.1');                
            //     // Nếu đang ở phần tử cuối cùng của vòng lặp, thêm onComplete cho timeline
            //     if (index === menuItemRefs.current.length - 1) {
            //         timeline.eventCallback('onComplete', () => {
            //             // Thực hiện các hành động sau khi tất cả các hiệu ứng đã hoàn tất
